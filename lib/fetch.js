    ...
    retry = function( error ) {
        ...
        if( shouldRetry ) {
            ...
            if( config.extCallback ) {
                config.extCallback( error , null , null );
            ...
            var req = method.request( config.url , function( res ) {
                ...
                res.on( 'end' , function() {
                    if( config.extCallback ) {
                        ...
                        config.extCallback( null , body , res.headers['content-type'] );
                        config.extCallback = null;
                        release();
                    }
                } );
            ...
            extCallback: function( err , body , mimetype ) {
                if( err ) {
                    ...
                    parse.parseProjectListPage( url , body , function( pageProjects ) {
                        ...
                        // for each project
                        projectsProcessing += pageProjects.length;
                        ...
                        } ); // end pageProjects.forEach

                        deferred.resolve( pageProjects );
                    } ); // end parseProjectsListPage callback
                } // end if-success
            }, // end extCallback
        }); // end fetchPage

        return deferred.promise;
    }; // end fetchDeferredProjectListPage
...
// REQUIRED config.project.url
// REQUIRED config.callback
// EXPORTS config.project.details
exports.fetchProjectDetails = function( config ) {
    exports.fetchPage({
        url: config.project.url,
        extCallback: function( err , body , mimetype ) {
            if( err ) {
                throw 'Unable to fetch project details for: ' + config.project.name;
            } else {
                parse.parseProjectPage( config.project.url , body , function( html ) {
                    config.project.details = html;
                    config.callback();
                } );
            }
        },
